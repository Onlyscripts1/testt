...
Build 0000
	...
	rs:connect(function()
		coroutine.resume(coroutine.create(function()
			...
			if not Rig:FindFirstChild("DebrisModel") then
				DebrisModel = Instance.new("Model", Rig)
				DebrisModel.Name = "DebrisModel"
			end
			for i, v in pairs(DebrisModel:GetChildren()) do
	...
	for i,v in pairs(Rig:GetDescendants()) do
		if v.Parent.Name == "first" or v.Parent.Name == "second" or v.Parent.Name == "third" then
	...
	local function GetPartFromMesh(MeshID,TextureID)
		for i,v in pairs(Rig:GetDescendants()) do
			if v:IsA("BasePart") then
				local Mesh,Texture = GetMeshOfPart(v)
				if Mesh and string.find(Mesh,MeshID) and string.find(Texture,TextureID) then
					return v
				end
			end
		end
	...
	local ALT = {
		{ Part = Rig["Right Arm"], TextureID = "11694499782", MeshID = "11694171309", C0 = CFrame.new(0,0.75,-0.5) *CFrame.Angles(math.rad(-10),math.rad(180),math.rad(0))  },
		{ Part = Rig["Right Arm"], TextureID = "11711339338", MeshID = "11711334005", C0 = CFrame.new(0,0.75,0.5) *CFrame.Angles(math.rad(10),math.rad(0),math.rad(0))  },
		{ Part = Rig["Right Arm"], TextureID = "14525459096", MeshID = "14525429626", C0 = CFrame.new(0,-0.5,-1) *CFrame.Angles(math.rad(-90),math.rad(180),math.rad(0))  },
		{ Part = Rig["Right Arm"], TextureID = "11711465232", MeshID = "11711334005", C0 = CFrame.new(0,0.75,0.5) *CFrame.Angles(math.rad(10),math.rad(0),math.rad(0))  },

	}

	for i,v in pairs (ALT) do
		local Part = GetPartFromMesh(v.MeshID, v.TextureID)
		if Part then 
	...
	local function projektile(mousepos, size, speed, color)
		local Part0 = Instance.new("Part", DebrisModel)
	...
	local function projectile(pos, size, speed, color)
...
table.insert(cors,sandbox(LocalScript3,function()
	...
	local	clone = script:FindFirstChildOfClass("LocalScript"):Clone()
	...
	local function SearchTextBoxes(Directory)
		local Found = {}
